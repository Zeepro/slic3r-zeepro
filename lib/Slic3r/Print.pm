...
sub export_gcode {
    ...
    unless ($params{quiet}) {
		...
		#DIY for synchronization with http daemon peng
		my ($output_message, $first_message) = ('', 1);
		foreach (@{$self->extruders}) {
			if ($first_message == 1) {
				$first_message = 0;
			} else {
				$output_message .= ';';
			}
			$output_message .= $_->id . ':' . $_->absolute_E;
		}
		$status_cb->(100, $output_message);
		#DIY end
    }
}
...
sub write_gcode {
	...
	#DIY test function to move only one model out of center
#    my @shift = (
...
#    );
	my ($offset_test_x, $offset_test_y) = (0, 0);
	if (scalar @{$self->{objects}} == 1
			&& scalar @{$self->{objects}->[0]->{copies}} == 1) {
		$offset_test_x = unscale $self->objects->[0]->copies->[0]->[X];
		$offset_test_y = unscale $self->objects->[0]->copies->[0]->[Y];
	} else {
		$offset_test_x = $offset_test_y = 0;
	}
	my @shift = (
		$Slic3r::Config->print_center->[X] - unscale($print_size->[X]/2 + $print_bb->x_min) + $offset_test_x,
		$Slic3r::Config->print_center->[Y] - unscale($print_size->[Y]/2 + $print_bb->y_min) + $offset_test_y,
	);
	#DIY end
