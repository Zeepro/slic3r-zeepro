...
#DIY http daemon filament synchronization
has 'info_used_filament'	=> (is => 'rw', default => undef);
#DIY end - PNI
...
sub export_gcode {
	...
	#DIY for synchronization with http daemon
	$status_cb->(100, $self->info_used_filament);
	#DIY end - PNI
}
...
#    }
    ...
    foreach my $extruder_id (@{$self->extruders}) {
		...
		#DIY http daemon filament synchronization
		my $sync_filament = $used_filament;
		if ($used_filament < 0) {
			print "negative extrusion found => $extruder_id: $used_filament\n";
			$sync_filament = 0;
		}
		if (defined ($self->info_used_filament)) {
			$self->info_used_filament($self->info_used_filament . ';' . $extruder_id . ':' . $sync_filament);
		}
		else {
			$self->info_used_filament($extruder_id . ':' . $sync_filament);
		}
		#DIY end - PNI
