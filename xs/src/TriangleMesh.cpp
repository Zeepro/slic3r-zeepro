...
//DIY rotation 360
void TriangleMesh::rotateX(double angle, float center_y, float center_z)
{
//	float center_y = (this->stl.stats.max.y + this->stl.stats.min.y) / 2;
//	float center_z = (this->stl.stats.max.z + this->stl.stats.min.z) / 2;
	
	this->translate(0, -center_y, -center_z);
	stl_rotate_x(&(this->stl), (float)angle);
	this->translate(0, +center_y, +center_z);
}

void TriangleMesh::rotateY(double angle, float center_x, float center_z)
{
//	float center_x = (this->stl.stats.max.x + this->stl.stats.min.x) / 2;
//	float center_z = (this->stl.stats.max.z + this->stl.stats.min.z) / 2;
	
	this->translate(-center_x, 0, -center_z);
	stl_rotate_y(&(this->stl), (float)angle);
	this->translate(+center_x, 0, +center_z);
}
//DIY end - PNI
